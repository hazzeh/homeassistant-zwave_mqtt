...
from tests.common import async_capture_events, setup_zwave
...
async def test_central_scene(hass, sent_messages):
    """Verify that scene_activated event from Central Scene contains correct labels."""
    receive_message = await setup_zwave(hass, "generic_network_dump.csv")
    events = async_capture_events(hass, "zwave_mqtt.scene_activated")
    ...
    # wait for the event
    count = 0
    while count < 5 and len(events) != 1:
        await asyncio.sleep(0.5)
        count += 1
    assert events[0].data["scene_id"] == 2
    assert events[0].data["scene_label"] == "Scene 2"
    assert events[0].data["scene_value_id"] == 3
    assert events[0].data["scene_value_label"] == "Key Held down"
